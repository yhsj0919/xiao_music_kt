package xyz.yhsj.cyf

import java.util.*

class KuwoMusic {
    fun search(keyword: String): List<HashMap<String, Any>> {

        val headers = hashMapOf(
            "referer" to "http://www.kuwo.cn/",
            "user-agent" to "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/142.0.0.0 Safari/537.36 Edg/142.0.0.0",
        )
        // 定义查询参数
        val params = hashMapOf(
            "vipver" to "1",
            "ft" to "music",
            "encoding" to "utf8",
            "issubtitle" to "1",
            "mobi" to "1",
            "all" to keyword,
            "pn" to 0,
            "rn" to 20
        )

        val ss = HttpUtils2.get(
            "https://www.kuwo.cn/search/searchMusicBykeyWord",
            params,
            headers = headers
        )

        val aa = ss.body()
        println(aa)
        val abslist = aa.toMap().get("abslist").json().toModel<List<Map<String, Any>>>()
        val musicList = abslist.map {


            hashMapOf<String, Any>(
                "id" to it["DC_TARGETID"].toString(),
                "pic" to "https://img2.kuwo.cn/star/albumcover/${it["web_albumpic_short"].toString().replace("120/", "300/")}",
                "name" to it["SONGNAME"].toString(),
                "artist" to it["ARTIST"].toString(),
                "album" to it["ALBUM"].toString(),

                )


        }

        println(musicList)
        return musicList

    }

    val first: String = "ylzsxkwm"

    private fun encode(id: String): String {
        val s =
            "user=0&android_id=0&prod=kwplayer_ar_5.1.0.0_B&corp=kuwo&newver=3&vipver=5.1.0.0&source=kwplayer_ar_5.1.0.0_B_jiakong_vh.apk&p2p=1&notrace=0&type=convert_url2&br=320kmp3&format=flac|mp3|aac&sig=0&rid=${id}&priority=bitrate&loginUid=0&network=WIFI&loginSid=0&mode=download"
        val bArr = s.toByteArray()
        val a2: ByteArray = d.a(bArr, bArr.size, first.toByteArray(), first.toByteArray().size)
        return String(b.a(a2, a2.size))
    }

    fun getUrl(id: String): String? {

        val downloadUrl = "http://nmobi.kuwo.cn/mobi.s?f=kuwo&q=${encode(id)}"


        val data = HttpUtils2.get(downloadUrl).body()

        var myUrl: String? = null
        data.split("\r\n").forEach {
            val item = it.split("=")
            if (item[0] == "url") {
                myUrl = item[1]
                return@forEach
            }
        }
        println(myUrl)
        return myUrl

    }

}